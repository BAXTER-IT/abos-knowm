...
import org.knowm.xchange.gemini.dto.enums.GeminiOrderFlags;
...
public class GeminiTradeServiceRaw extends GeminiBaseService {
   ...
   * @param exchange
   */
  public GeminiTradeServiceRaw(GeminiExchange exchange) {
  ...
  public GeminiOrderStatusResponse[] getGeminiOpenOrders() throws IOException {
    try {
      GeminiOrderStatusResponse[] activeOrders =
          geminiAuthenticatedV1.activeOrders(
              apiKey,
              payloadCreator,
              signatureCreator,
  ...
  public GeminiOrderStatusResponse placeGeminiLimitOrder(
    ...
    try {
      GeminiOrderStatusResponse newOrder =
          geminiAuthenticatedV1.newOrder(apiKey, payloadCreator, signatureCreator, request);
      return newOrder;
    } catch (GeminiException e) {
      throw handleException(e);
    }
  ...
  public boolean cancelGeminiOrder(String orderId) throws IOException {
    try {
      geminiAuthenticatedV1.cancelOrders(
          apiKey,
          payloadCreator,
          signatureCreator,
  ...
  public GeminiCancelAllOrdersResponse cancelAllGeminiOrders(boolean sessionOnly, String accountId)
    ...
    try {
      if (sessionOnly) {
        return geminiAuthenticatedV1.cancelAllSessionOrders(
            apiKey,
            payloadCreator,
            signatureCreator,
        ...
        return geminiAuthenticatedV1.cancelAllOrders(
            apiKey,
            payloadCreator,
            signatureCreator,
            new GeminiCancelAllOrdersRequest(
  ...
  public GeminiOrderStatusResponse getGeminiOrderStatus(String orderId) throws IOException {
    try {
      GeminiOrderStatusResponse orderStatus =
          geminiAuthenticatedV1.orderStatus(
              apiKey,
              payloadCreator,
              signatureCreator,
  ...
  public GeminiOrderStatusResponse getGeminiOrderStatus(OrderQueryParams params)
    ...
    try {
      return geminiAuthenticatedV1.orderStatus(
          apiKey,
          payloadCreator,
          signatureCreator,
          orderId == null
  ...
  public List<GeminiTradeResponse> getGeminiTradeHistory(String symbol, Long timestamp, Integer limit)
      throws IOException {
    try {
      GeminiPastTradesRequest request = GeminiPastTradesRequest.builder()
          .symbol(symbol)
          .timestamp(timestamp)
          .limit(limit)
          .build();
      return geminiAuthenticatedV1.pastTrades(
          apiKey,
          payloadCreator,
          signatureCreator,
          request);
  ...
  public GeminiOrderStatusResponse heartBeat() throws IOException {
    try {
      return geminiAuthenticatedV1.heartBeat(
          apiKey,
          payloadCreator,
          signatureCreator,
          new GeminiNonceOnlyRequest(
