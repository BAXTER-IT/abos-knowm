...
public class OkexStreamingService extends JsonNettyStreamingService {
  ...
  public void login() throws JsonProcessingException {
    ...
    try {
      ...
      final SecretKey secretKey =
          new SecretKeySpec(xSpec.getSecretKey().getBytes(StandardCharsets.UTF_8),
    ...
    OkexLoginMessage.LoginArg loginArg = new OkexLoginMessage.LoginArg(xSpec.getApiKey(),
  ...
  private OkexSubscribeMessage.SubscriptionTopic getTopic(String channelName) {
    if (channelName.contains(ORDERBOOK5)) {
      return buildTopic(channelName, ORDERBOOK5, null);
    }

    if (channelName.contains(ORDERBOOK)) {
      return buildTopic(channelName, ORDERBOOK, null);
    }

    if (channelName.contains(TRADES)) {
      return buildTopic(channelName, TRADES, null);
    }

    if (channelName.contains(TICKERS)) {
      return buildTopic(channelName, TICKERS, null);
    }

    if (channelName.contains(USERTRADES)) {
      return buildTopic(channelName, USERTRADES, OkexInstrumentType.ANY);
    }

    if (channelName.contains(FUNDING_RATE)) {
      return buildTopic(channelName, FUNDING_RATE, null);
    }

    if (channelName.contains(POSITIONS)) {
      return buildTopic(channelName, POSITIONS, OkexInstrumentType.ANY);
    }
    throw new NotYetImplementedForExchangeException(
        "ChannelName: " + channelName + " has not implemented yet on " + this.getClass()
            .getSimpleName());

  }

  private OkexSubscribeMessage.SubscriptionTopic buildTopic(
      String channelName,
      String channel,
      OkexInstrumentType instType
  ) {
    var builder = OkexSubscribeMessage.SubscriptionTopic.builder()
        .channel(channel)
        .instId(channelName.replace(channel, ""))
        .extraParams(
            "{\"updateInterval\":\"0\"}"); // do not receive 5s snapshots, only event updates

    if (instType != null) {
      builder.instType(instType);
    }
