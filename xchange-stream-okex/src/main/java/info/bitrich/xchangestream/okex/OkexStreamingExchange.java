...
public class OkexStreamingExchange extends OkexExchange implements StreamingExchange {
  ...
  public Completable connect(ProductSubscription... args) {
    streamingService = new OkexStreamingService(getApiUrl(), exchangeSpecification);
    streamingAccountService = new OkexStreamingAccountService(streamingService, exchangeMetaData);
    streamingMarketDataService = new OkexStreamingMarketDataService(streamingService);
    streamingTradeService = new OkexStreamingTradeService(streamingService, exchangeMetaData);
  ...
  private String getApiUrl() {
    ...
    if (useSandbox()) {
      apiUrl = (this.exchangeSpecification.getApiKey() == null) ? SANDBOX_WS_PUBLIC_CHANNEL_URI
          : SANDBOX_WS_PRIVATE_CHANNEL_URI;
    } else {
      apiUrl =
          (this.exchangeSpecification.getApiKey() == null) ? userAws ? AWS_WS_PUBLIC_CHANNEL_URI
